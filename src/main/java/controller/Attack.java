...
public class Attack implements TryToDoAnAction{
	...
	public String attackMove(){
		...
		for(int i = 0; i < current.getPlayers().size()-1; i++){
			...
			if(isDefendable(attacked)){
				...
				if(gameStatus.getPlayerPlay().getType().equals(PlayerType.ALIEN) && attacked.getType().equals(PlayerType.HUMAN)){
				...
				for(int j = 0; j < attacked.getItem().size(); j++){ //scarto tutte le carte oggetto del giocatore eliminato
					gameStatus.getGame().getItemCards().discard(attacked.removeItem(j));
				}	
			}
		}
	...
	private boolean isDefendable(Player attacked){
		for(int j = 0; j < attacked.getItem().size(); j++){
			if(attacked.getItem().get(j).getType().equals(ItemCardType.DEFENSE)){
				gameStatus.getGame().getItemCards().discard(attacked.removeItem(j)); //scarto carta difesa  
				return true;
			}
		}
		return false;	
	...
	public String doAction(DTOTurn dtoTurn) {
		if(gameStatus.isMove()&&gameStatus.isAttack()==false&&gameStatus.isSolveSectorDuty()){	//attacco alieno
			return attackMove();
		}
