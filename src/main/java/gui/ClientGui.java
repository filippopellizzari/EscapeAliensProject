...
/**
 * This class starts the client who plays in GUI mode; choose of connection and
 * map is through console
 * 
 * @author Filippo
 *
 */
public class ClientGui {
	...
	/**
	 * This method starts client; after choose of connection(RMI or Socket) and
	 * of map (Fermi, Galilei, Galvani), client waits for a game available
	 * during a certain interval of time. If a game is available, graphic is
	 * created and the game table is displayed.
	 * 
	...
	public void startClientGui() throws UnknownHostException,
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				DefaultTableModel dataModel = lp.getDiscardPanel()
						.getDataModel();
				...
				try {
					lp.getSouthWestPanel()
							.getTextArea()
							.append("Info Giocatore\n" + "Numero: "
									+ (cd.getView().getNumberPlayer() + 1)
									+ "\n");
				...
				try {
					lp.getSouthWestPanel().getTextArea()
							.append("Tipo: " + cd.getView().getPlayerType());
				} catch (InterruptedException e1) {
					System.out.println("Problem Get View: " + e1);
