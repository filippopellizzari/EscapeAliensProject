...
public class ThreadEndTurn implements Runnable {
	...
	/**
	 * Until the end of the game this thread for each players' turn create a
	 * thread, wait until the game's turn changes then then if the turn is
	 * complete begins again, else completes the player's turn
	 */
	...
	public void run() {
		try {
			...
			do {
				...
				turn = gameDescription.getController().getRound(); // imposto il
																	// nuovo
																	// turno e
																	// giocatore
				numberPlayer = gameDescription.getController()
